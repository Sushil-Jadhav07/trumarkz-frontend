...
import {
  ...
  AlertTriangle, ArrowRight, ArrowLeft, Shield, ChevronRight,
} from 'lucide-react';
...
const ActionModal = ({ isOpen, onClose, product, action, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    setSubmitting(true);
    try {
      await verificationAPI.approveRejectWarranty(product.product_id || product.id, action, reason.trim());
...
const ProductRow = ({ product, index, onAction }) => (
  ...
  >
    ...
    <td className="px-5 py-3.5">
      ...
      {(product.warranty_status === 'approved' || !product.warranty_status) && (
        <span className="text-xs text-gray-400 font-inter">—</span>
      )}
      {product.warranty_status === 'rejected' && product.warranty_reason && (
        <span className="text-xs text-gray-400 font-inter italic truncate max-w-[120px] block" title={product.warranty_reason}>
          {product.warranty_reason}
        </span>
      )}
    </td>
  </motion.tr>
...
export const ProductWarrantyAdmin = () => {
  ...
  const [inputVal, setInputVal] = useState(batchIdFromUrl);
  ...
  // Auto-load when arriving from a batch list link (?batch_id=...) instead of
  // requiring the admin to retype the ID they just clicked.
  useEffect(() => {
    if (batchIdFromUrl) fetchWarranty(batchIdFromUrl);
                  ...
                  <table className="w-full min-w-[900px] font-inter">
                    <thead>
                      <tr className="border-b border-blue-100 bg-blue-50/80">
                        {['Product', 'Purchase Date', 'Warranty Start', 'Warranty End', 'Status', 'Action'].map((h) => (
