...
const DeleteDocumentModal = ({ doc, deleting, onClose, onConfirm }) => (
  <Modal isOpen={!!doc} onClose={deleting ? undefined : onClose} title="Delete Document" size="sm">
    {doc && (
      <div className="space-y-4">
        <div className="flex items-start gap-3 rounded-xl border border-red-100 bg-red-50 p-4">
          <AlertTriangle size={18} className="mt-0.5 shrink-0 text-red-500" />
          <p className="font-inter text-sm text-red-700">
            <span className="font-medium capitalize">{readable(doc.document_label)}</span> will be permanently deleted. This cannot be undone.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            ...
            onClick={onClose}
            disabled={deleting}
            className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm font-medium text-gray-600 font-inter transition-colors hover:bg-gray-50 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            ...
            onClick={onConfirm}
            disabled={deleting}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-red-500 px-4 py-2.5 text-sm font-medium text-white font-inter transition-colors hover:bg-red-600 disabled:opacity-60"
          >
...
export const RecordDetail = () => {
  ...
  const handleDeleteDocument = async () => {
    if (!docToDelete || !record) return;
    setDeletingDoc(true);
    try {
      await verificationAPI.deleteBatchUserDocument(record.batch_id, record.id, docToDelete.id);
      setRecord((current) => current ? {
        ...current,
        documents: (current.documents || []).filter((d) => d.id !== docToDelete.id),
      } : current);
